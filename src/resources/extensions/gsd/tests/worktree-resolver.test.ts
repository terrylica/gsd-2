...
import { WorktreeResolver, type WorktreeResolverDeps, type NotifyCtx } from "../worktree-resolver.js";
...
function makeDeps(overrides?: Partial<WorktreeResolverDeps>): WorktreeResolverDeps & { calls: CallLog[] } {
  ...
  const deps: WorktreeResolverDeps & { calls: CallLog[] } = {
    ...
    captureIntegrationBranch: (basePath: string, mid: string | undefined, opts?: { commitDocs?: boolean }) => {
      calls.push({ fn: "captureIntegrationBranch", args: [basePath, mid, opts] });
    },
    ...overrides,
  };
...
test("projectRoot returns originalBasePath when set", () => {
  const s = makeSession({ basePath: "/project/.gsd/worktrees/M001", originalBasePath: "/project" });
  const resolver = new WorktreeResolver(s, makeDeps());
...
test("enterMilestone creates new worktree when none exists", () => {
  ...
  assert.ok(ctx.messages.some(m => m.level === "info" && m.msg.includes("Entered worktree")));
});
...
test("enterMilestone does NOT update basePath on creation failure", () => {
  ...
  assert.ok(ctx.messages.some(m => m.level === "warning" && m.msg.includes("disk full")));
});
...
test("exitMilestone passes preserveBranch option", () => {
  ...
  const deps = makeDeps({
    isInAutoWorktree: () => true,
    teardownAutoWorktree: (_basePath: string, _mid: string, opts?: { preserveBranch?: boolean }) => {
      preserveOpts = opts;
    },
  });
  const ctx = makeNotifyCtx();
...
test("exitMilestone still resets basePath even if auto-commit fails", () => {
  ...
  const deps = makeDeps({
    isInAutoWorktree: () => true,
    autoCommitCurrentBranch: () => { throw new Error("commit error"); },
  });
  const ctx = makeNotifyCtx();
  const resolver = new WorktreeResolver(s, deps);
...
test("mergeAndExit in worktree mode restores to project root on merge failure", () => {
  ...
  assert.ok(ctx.messages.some(m => m.level === "warning" && m.msg.includes("conflict in main")));
...
test("mergeAndExit in branch mode handles merge failure gracefully", () => {
  ...
  const deps = makeDeps({
    ...
    getCurrentBranch: () => "milestone/M001",
    autoWorktreeBranch: () => "milestone/M001",
    mergeMilestoneToMain: () => { throw new Error("branch merge conflict"); },
  });
  const ctx = makeNotifyCtx();
  const resolver = new WorktreeResolver(s, deps);
  ...
  assert.ok(ctx.messages.some(m => m.level === "warning" && m.msg.includes("branch merge conflict")));
});
...
test("mergeAndEnterNext calls mergeAndExit then enterMilestone", () => {
  ...
  const deps = makeDeps({
    ...
    mergeMilestoneToMain: (basePath: string, milestoneId: string, _roadmap: string) => {
...
test("mergeAndEnterNext enters next milestone even if merge fails", () => {
  ...
  const deps = makeDeps({
    ...
    getIsolationMode: () => "worktree",
    shouldUseWorktreeIsolation: () => true,
    mergeMilestoneToMain: () => { throw new Error("merge failed"); },
    getAutoWorktreePath: () => null,
  ...
  assert.equal(s.basePath, "/project/.gsd/worktrees/M002");
  assert.ok(ctx.messages.some(m => m.level === "warning" && m.msg.includes("merge failed")));
  assert.ok(ctx.messages.some(m => m.level === "info" && m.msg.includes("Entered worktree")));
});
